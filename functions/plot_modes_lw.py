...
def plot_all_modes_separated(case, num_mode, name_out=False):
    ...
    for dtype in ["U","p"]:
        ...
        # PNG files for all modes or for a single mode num_mode (=1 or 2 or ...or nmodes)
        if ( num_mode == -1 ):
            # all modes in separated files
            list_range1 = range( nmodes )
            list_range2 = range( 1 )
        else:
            if ( num_mode == 0 ):
                # all modes in a single file
                list_range1 = range( 1 )
                list_range2 = range( nmodes )
            else:
                # single mode num_mode (=1 or 2 or ...or nmodes) in one file
                list_range1 = range(num_mode-1, num_mode, 1)
                list_range2 = range(num_mode-1, num_mode, 1)

        for N1 in (list_range1):

            if ( num_mode == 0 ):
                nrow, ncol = case.get_size_mode_mozaic(nmodes)
                fig_title = "All modes"
            else:
                nrow, ncol = case.get_size_mode_mozaic(1)
                fig_title = "Mode "+str(N1+1)
            ...
            for N2 in (list_range2):
                ...
                ss = AXS_N.get_subplotspec()

                # mplcyberpunk.make_lines_glow(AXS_N)
                # Title
                AXS_N.set_title(f"Mode {N+1}")
            ...
            #for N2 in (list_range2):
                    ...
                    #N = N1
                    #AXS_N = axs[0]
                #print("N1=",N1,"  N2=",N2, "  xlabel=",AXS_N.set_xlabel)
            ...
            if ( num_mode == 0 ):
                ...
                if name_out:
                    name_save = f"{dtype}_{name_out}_mode{N1}"
                else:
                    #name_save = f"{dtype}_modes_{case.name}_mode{N1}"
                    name_save = f"{dtype}_{case.name}_mode{N1}"
