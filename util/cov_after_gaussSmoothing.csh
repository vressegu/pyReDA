...
#          files in [DIR0/ROM_PIV/DIR1/cov_after_gaussSmoothing] : list_sliceZ_fic_time.txt and other U*.txt files
...
foreach D ( ${All_D} ) 
  ...
  if (!(-e ${dir_work}/cov_after_gaussSmoothing/list_sliceZ_fic_time.txt)) then
    set error = "${dir_work}/cov_after_gaussSmoothing/list_sliceZ_fic_time.txt NOT FOUND"
    WRITE_LINE; WRITE_ERROR "${error}"; WRITE_LINE
    exit()
  endif

  if (!(-e ${dir_work}/cov_after_gaussSmoothing/list_sliceY_fic_time.txt)) then
    set error = "${dir_work}/cov_after_gaussSmoothing/list_sliceY_fic_time.txt NOT FOUND"
    WRITE_LINE; WRITE_ERROR "${error}"; WRITE_LINE
    exit()
  endif
  ...
  set Zslice = ` cat ${dir_work}/openfoamDNS_to_pseudoPIV.info | grep "ZSLICEZ_VALUE=" | awk -F'=' '{ print $2 }' `
  set Yslice = ` cat ${dir_work}/openfoamDNS_to_pseudoPIV.info | grep "DNS_ycyl=" | awk -F'=' '{ print $2 }' `

  cd ${dir_work}/cov_after_gaussSmoothing
  
  foreach slice ( sliceZ sliceY )

    set Utxt_first = `  cat list_${slice}_fic_time.txt | grep U | head -1 `
    set Nrow = ` cat ${Utxt_first} | wc -l `
    set Ntime = ` cat list_${slice}_fic_time.txt | wc -l | awk '{ print $1-1 }' `
  ...
  \cp   ~/Bureau/MORAANE/pyReDA/util/cov_after_gaussSmoothing_stat.C ${dir_work_up}/util
    ...
    ./cov_after_gaussSmoothing_stat list_${slice}_fic_time.txt ${Nrow} ${Ntime}
    ...
    if ( ${slice} == sliceZ ) then
      ...
      cat XYcrop.txt | \
       ...
       if -e XZ.txt \rm XZ.txt
      cat XZcrop.txt | \
        awk -v P_d=${PIV_Dcyl} -v D_d=${DNS_Dcyl} -v D_x0=${DNS_crop_x0} -v P_x0=${PIV_xmin} '{ \
        printf("%10.6f %10.6f\n", ($1-D_x0+0.)*(P_d+0.)/(D_d+0.)+P_x0+0., ($2)*(P_d+0.)/(D_d+0.)+0) }' > XZ.txt
    endif
    ...
    if ( ${slice} == sliceZ ) then
      ...
      if -e cov_zz.txt \rm cov_zz.txt
      cat cov_zz.csv | grep -v cov | awk  -v A=${A_mat_diag} '{ print $1+A**2 }' > cov_zz.txt
      if -e cov_zx.txt \rm cov_zx.txt
      cat cov_zx.csv | grep -v cov > cov_zx.txt
      if -e tmp.txt \rm tmp.txt
      paste cov_xx.txt  cov_zz.txt cov_zx.txt > tmp.txt
    ...
    if ( ${slice} == sliceZ ) then
      ...
      set PIV_file_new = Inv_COVxz.dat
    endif
    ...
    if ( ${slice} == sliceZ ) then
      ...
      echo "# Cov = ((cov_xx+A2,cov_zx),(cov_zx,cov_zz+A2)) " >> ${PIV_file_new}
      echo "#   with A2 = ${A_mat_diag}**2" >> ${PIV_file_new}
      echo "# => InvCov = 1/Cov = ((cov_zz+A2,-cov_zx), (-cov_zx,cov_xx+A2))/det " >> ${PIV_file_new}
      echo "# x   z  InvCov[1][1]   InvCov[2][2]   InvCov[1][2] " >> ${PIV_file_new}
      paste XZ.txt COV.txt >> ${PIV_file_new}
    ...
    if ( ${slice} == sliceZ ) then
      ...
      if -e PIV_new_zinv.txt \rm PIV_new_zinv.txt
      cat ${PIV_file_new} | grep -v "#" | \
        awk '{ x=$2+0. ; if (NF!=0) printf("%.0f\n", 1000000.*x) }' | sort -n -r  | uniq | \
        awk '{ x=$1+0. ; printf("%.6f\n",x/1000000.)}' > PIV_new_zinv.txt
      set All_PIV_new_zinv = ` cat PIV_new_zinv.txt `
    ...
    if ( ${slice} == sliceZ ) then
      ...
      echo "# => InvCov = 1/Cov = ((cov_zz+A2,-cov_zx), (-cov_zx,cov_xx+A2))/det " >> ${PIV_file_new}
      echo "# x   y  InvCov[1][1]   InvCov[2][2]   InvCov[1][2] " >> ${PIV_file_new}
      foreach z ( ${All_PIV_new_zinv} )
        cat tmp.txt | awk -v z=${z} -v dx_max=${dx_max} '{ if ($2==z) print $1+dx_max, $0 }' | \
        sort -n | uniq | \
        awk '{ for (i=2;i<NF; i++) printf("%s ",$i); print $NF }' >> ${PIV_file_new}
      end
    ...
    # visu gnuplot
    set fic_gnp = ` echo ${PIV_file_new} | sed s/"\.dat"/"\.gnp"/g `
    ...
    if ( ${slice} == sliceZ ) then
      ...
      echo 'set title "pvbatch on ROM '${D}' : grid1 to uniform grid2 (Gaussian Smoothing then Slice Y='${Yslice}')" noenhanced' >> ${fic_gnp}
      echo 'unset arrow' >> ${fic_gnp}
      ...
      # cylinder : black rectangle width W=1.0
    ...
    if ( ${slice} == sliceZ ) then
      ...
      echo 'set output "PIV_new_covInv'${slice}'_Uxx.png"' >> ${fic_gnp}
      echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][PIV_DNS_yr1-0.01:PIV_DNS_yr2+0.01] \' >> ${fic_gnp}
      echo '  "'${PIV_file_new}'" u (($1-(PIV_xcyl))/PIV_Dcyl):(($2-(PIV_ycyl))/PIV_Dcyl):3 w p pt 5 ps 1 palette title "M[1][1] with M=inv(  (  (cov(ux*ux)+'${A_mat_diag}'**2), cov(ux*uy)),   ((cov(ux*uy), (cov(uy*uy)+'${A_mat_diag}'**2) )  )"' >> ${fic_gnp}
      echo 'set output "PIV_new_covInv'${slice}'_Uyy.png"' >> ${fic_gnp}
      echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][PIV_DNS_yr1-0.01:PIV_DNS_yr2+0.01] \' >> ${fic_gnp}
      echo '  "'${PIV_file_new}'" u (($1-(PIV_xcyl))/PIV_Dcyl):(($2-(PIV_ycyl))/PIV_Dcyl):4 w p pt 5 ps 1 palette title "M[2][2] with M=inv(  (  (cov(ux*ux)+'${A_mat_diag}'**2), cov(ux*uy)),   ((cov(ux*uy), (cov(uy*uy)+'${A_mat_diag}'**2) )  )"' >> ${fic_gnp}
      echo 'set cbrange [-100:100]' >> ${fic_gnp}
      echo 'set output "PIV_new_covInv'${slice}'_Uxy.png"' >> ${fic_gnp}
      echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][PIV_DNS_yr1-0.01:PIV_DNS_yr2+0.01] \' >> ${fic_gnp}
      echo '  "'${PIV_file_new}'" u (($1-(PIV_xcyl))/PIV_Dcyl):(($2-(PIV_ycyl))/PIV_Dcyl):5 w p pt 5 ps 1 palette title "M[1][2] with M=inv(  (  (cov(ux*ux)+'${A_mat_diag}'**2), cov(ux*uy)),   ((cov(ux*uy), (cov(uy*uy)+'${A_mat_diag}'**2) )  )"' >> ${fic_gnp}

    else

      echo 'set output "PIV_new_covInv'${slice}'_Uxx.png"' >> ${fic_gnp}
      echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
      echo '  "'${PIV_file_new}'" u (($1-(PIV_xcyl))/PIV_Dcyl):(($2)/PIV_Dcyl):3 w p pt 5 ps 1 palette title "M[1][1] with M=inv(  (  (cov(ux*ux)+'${A_mat_diag}'**2), cov(ux*uz)),   ((cov(ux*uz), (cov(uz*uz)+'${A_mat_diag}'**2) )  )"' >> ${fic_gnp}
      echo 'set output "PIV_new_covInv'${slice}'_Uzz.png"' >> ${fic_gnp}
      echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
      echo '  "'${PIV_file_new}'" u (($1-(PIV_xcyl))/PIV_Dcyl):(($2)/PIV_Dcyl):4 w p pt 5 ps 1 palette title "M[2][2] with M=inv(  (  (cov(ux*ux)+'${A_mat_diag}'**2), cov(ux*uz)),   ((cov(ux*uz), (cov(uz*uz)+'${A_mat_diag}'**2) )  )"' >> ${fic_gnp}
      echo 'set cbrange [-100:100]' >> ${fic_gnp}
      echo 'set output "PIV_new_covInv'${slice}'_Uxz.png"' >> ${fic_gnp}
      echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
      echo '  "'${PIV_file_new}'" u (($1-(PIV_xcyl))/PIV_Dcyl):(($2)/PIV_Dcyl):5 w p pt 5 ps 1 palette title "M[1][2] with M=inv(  (  (cov(ux*ux)+'${A_mat_diag}'**2), cov(ux*uz)),   ((cov(ux*uz), (cov(uz*uz)+'${A_mat_diag}'**2) )  )"' >> ${fic_gnp}
    ...
    echo 'set output "cov'${slice}'_xx.png" ' >> visu_cov.gnp
    ...
    echo '   ' >> visu_cov.gnp
    echo 'set output "cov'${slice}'_yy.png" ' >> visu_cov.gnp
    echo 'plot \' >> visu_cov.gnp
    ...
    echo ' ' >> visu_cov.gnp
    echo 'set output "cov'${slice}'_zz.png" ' >> visu_cov.gnp
    echo 'plot \' >> visu_cov.gnp
    ...
    echo '   ' >> visu_cov.gnp
    echo 'set output "cov'${slice}'_xy.png" ' >> visu_cov.gnp
    echo 'plot \' >> visu_cov.gnp
    ...
    echo ' ' >> visu_cov.gnp
    echo 'set output "cov'${slice}'_yz.png" ' >> visu_cov.gnp
    echo 'plot \' >> visu_cov.gnp
    ...
    echo '   ' >> visu_cov.gnp
    echo 'set output "cov'${slice}'_zx.png" ' >> visu_cov.gnp
    echo 'plot \' >> visu_cov.gnp
