...
#                	0              # code_adim
#                	1.             # Zslice (no more defined by user since 15/07/25 : Zslice=(Zmin+Zmax)/2)
#                	TEST t=0       # DNS_info
...
#                	1	#    code_view_sliceZ_Ux1 : =1 if PNG file showing Ux(Z=Zslice) openfoam result
#                	1	#    code_view_sliceZ_Uy1 : =1 if PNG file showing Uy(Z=Zslice) openfoam result
...
#                	1	#    code_view_sliceZ_Ux2 : =1 if PNG file showing Ux(Z=Zslice) openfoam after smoothing
#                	1	#    code_view_sliceZ_Uy2 : =1 if PNG file showing Uy(Z=Zslice) openfoam after smoothing
#                	0	#    code_csv_sliceZ_Ux1 : =1 if CSV file showing Ux(Z=Zslice) openfoam result
#                	0	#    code_csv_sliceZ_Uy1 : =1 if CSV file showing Uy(Z=Zslice) openfoam result
#                	1	#    code_csv_sliceZ_Ux2 : =1 if CSV file showing Ux(Z=Zslice) openfoam after smoothing
#                	1	#    code_csv_sliceZ_Uy2 : =1 if CSV file showing Uy(Z=Zslice) openfoam after smoothing
#                	1	#    code_csv_sliceZ_Uz2 : =1 if CSV file showing Uz(Z=Zslice) openfoam after smoothing
#
#                 # since 15/07/25 a second plane Y=cte centered on cylinder is added : code_... are the same as code_..._sliceZ_...
#                	
#
#      B.5) optional files
#
...
#      5) Zslice = 1. (value is fixed to Z=(Zmin+Zmax) since 15/07/25
...
#
#  since 15/07/25 : one argument added for Yslice
#    unchanged for arg1 to arg5
#       6) Yslice = 1. (value is fixed to Y=(Ymin+Ymax) since 15/07/25
#    DNS openfoam directories INFO : for example, name of directory (mean, ...) and time step
#       7) DNS_info = "residualSpeed_2 t=100"
#    PIV model file :
#       8) PIV_file_model = util/B0001.dat
#    Adding column IsValid like in PIV file : by default value=1 for this column
#       9) IsValid_ON = 1 (yes) =0 (no)
#
...
#     0
#  since 15/07/25 : one argument added for Yslice
#   Script command example : noise ON with max(noise)=2%, adim=OK, Zslice=1.75, Yslice=2.0, ..., IsValid OFF :
#     tcsh openfoamDNS_to_pseudoPIV_VR.csh \
#        openfoamDNS_to_pseudoPIV_param.txt \
#        residualSpeed_2 \
#        2. \
...
alias pvbatch '/usr/local/ParaView-5.13.1-MPI-Linux-Python3.10-x86_64/bin/pvbatch'
...
## Y value for slice defining FakePIV (added since 15/07/25)

# Y slice : value  from file parameters
set Yslice = ` cat ${fic_param} | grep "Yslice" | awk -F'#' '{ print $1 }' `
...
# if ( $6 != "" ) set DNS_info = $6 : modified since 15/07/25
if ( $7 != "" ) set DNS_info = $7
...
# if ( $7 != "" ) set PIV_file_model = $7 : modified since 15/07/25
if ( $8 != "" ) set PIV_file_model = $8
...
# if ( $8 != "" ) set IsValid_ON = $8 : modified since 15/07/25
if ( $9 != "" ) set IsValid_ON = $9
...
if -e ${PIV_file_model} then
  ...
  foreach var ( DNS_crop_Nx DNS_crop_Ny DNS_crop_Nz )
  
    if ( ${var} == DNS_crop_Nx ) set x = ${DNS_crop_Nx}
    if ( ${var} == DNS_crop_Ny ) set x = ${DNS_crop_Ny}
    if ( ${var} == DNS_crop_Nz ) set x = ${DNS_crop_Nz}
...
set DNS_crop_Nz = ` cat DNS_to_FakePIV_info.txt | grep "# " | grep " DNS_crop_Nz" | awk '{ print $1 }' `

if ( ${DNS_crop_Nz} == 0 ) then
  echo "\!\!\! The number of points for Fake PIV on Z direction (Yslice) is not defined => do update [DNS_to_FakePIV_info.txt] file \!\!\!"
  exit()
endif
...
echo "" >> openfoamDNS_to_pseudoPIV.info
echo "   -> PIV from DNS geometry (Zslice): " >> openfoamDNS_to_pseudoPIV.info
...
echo -n "      [ ${DNS_crop_Nx} x ${DNS_crop_Ny} ] points" >> openfoamDNS_to_pseudoPIV.info
echo " -> Yslice : [ ${DNS_crop_Nx} x ${DNS_crop_Nz} ] points" >> openfoamDNS_to_pseudoPIV.info
echo "" >> openfoamDNS_to_pseudoPIV.info
...
# DNS_crop_Nz redefined from DNS_crop_Nx and DNS_crop_Ny
set PIV_dx = ` echo ${PIV_Lx} ${DNS_crop_Nx} | awk '{ print $1/$2 }' `
set PIV_dy = ` echo ${PIV_Ly} ${DNS_crop_Ny} | awk '{ print $1/$2 }' `
set DNS_crop_Nz = ` echo ${PIV_dx} ${PIV_dy} ${PIV_Dcyl} ${DNS_Lz} ${DNS_Dcyl} | awk '{ d=$1/$3; if ($1<$2) d=$2/$3; l=$4/$5; printf("%.0f",l/d) }' `

# updating openfoamDNS_to_pseudoPIV.info
set N = ` cat -n openfoamDNS_to_pseudoPIV.info | grep "DNS_crop_Nz=" | awk '{ print $1 }' `
if ( ${N} != "" ) then
  \mv openfoamDNS_to_pseudoPIV.info tmp.txt
  awk -v N=${N} '{ if (NR<N) print $0 }' tmp.txt > openfoamDNS_to_pseudoPIV.info
  echo "DNS_crop_Nz=${DNS_crop_Nz}" >> openfoamDNS_to_pseudoPIV.info
  awk -v N=${N} '{ if (NR>N) print $0 }' tmp.txt >> openfoamDNS_to_pseudoPIV.info
else
  echo "DNS_crop_Nz=${DNS_crop_Nz}" >> openfoamDNS_to_pseudoPIV.info
endif

set DNS_crop_Rz = ` echo ${DNS_crop_Nz} | awk '{ print $1-1 }' `
set N = ` cat -n openfoamDNS_to_pseudoPIV.info | grep "DNS_crop_Rz=" | awk '{ print $1 }' `
if ( ${N} != "" ) then
  \mv openfoamDNS_to_pseudoPIV.info tmp.txt
  awk -v N=${N} '{ if (NR<N) print $0 }' tmp.txt > openfoamDNS_to_pseudoPIV.info
  echo "DNS_crop_Rz=${DNS_crop_Rz}" >> openfoamDNS_to_pseudoPIV.info
  awk -v N=${N} '{ if (NR>N) print $0 }' tmp.txt >> openfoamDNS_to_pseudoPIV.info
  \rm tmp.txt
else
  echo "DNS_crop_Rz=${DNS_crop_Rz}" >> openfoamDNS_to_pseudoPIV.info
...
# updating DNS_to_FakePIV_info.txt
set N = ` cat -n DNS_to_FakePIV_info.txt | grep "# DNS_crop_Nz" | awk '{ print $1 }' `
if ( ${N} != "" ) then
  \mv DNS_to_FakePIV_info.txt tmp.txt
  awk -v N=${N} '{ if (NR<N) print $0 }' tmp.txt > DNS_to_FakePIV_info.txt
  echo "${DNS_crop_Nz}              # DNS_crop_Nz" >> DNS_to_FakePIV_info.txt
  awk -v N=${N} '{ if (NR>N) print $0 }' tmp.txt >> DNS_to_FakePIV_info.txt
  \rm tmp.txt
else
  echo "${DNS_crop_Nz}              # DNS_crop_Nz" >> DNS_to_FakePIV_info.txt
...
echo "      X=[${DNS_xmin}:${DNS_xmax}] Y=[${DNS_ymin}:${DNS_ymax}] Z=[${DNS_zmin}:${DNS_zmax}]" >> openfoamDNS_to_pseudoPIV.info
echo "      Box(Zslice)=[ ${DNS_Lx} x ${DNS_Ly} ] Box(Yslice)=[ ${DNS_Lx} x ${DNS_Lz} ]" >> openfoamDNS_to_pseudoPIV.info
...
set DNS_crop_Lz = ` echo ${DNS_zmax} ${DNS_zmin} ${DNS_Dcyl} | awk '{ P_M=$1+0.; P_m=$2+0.; P_d=$3+0.; print (P_M-P_m)/P_d }' `
...
echo "       pointVolumeInterpolator_U*.Source.Origin = Box(Zslice)[${DNS_crop_x0}, ${DNS_crop_y0}] Box(Yslice)[${DNS_crop_x0}, ${DNS_crop_z0}]" >> openfoamDNS_to_pseudoPIV.info
echo "       pointVolumeInterpolator_U*.Source.Scale = Box(Zslice)[${DNS_crop_Lx}, ${DNS_crop_Ly}] Box(Yslice)[${DNS_crop_Lx}, ${DNS_crop_Lz}]" >> openfoamDNS_to_pseudoPIV.info
echo "       pointVolumeInterpolator_U*.Source.RefinementMode = 'Use resolution'" >> openfoamDNS_to_pseudoPIV.info
echo "       pointVolumeInterpolator_U*.Source.Resolution = Box(Zslice)[${DNS_crop_Rx}, ${DNS_crop_Ry}] Box(Yslice)[${DNS_crop_Rx}, ${DNS_crop_Rz}]" >> openfoamDNS_to_pseudoPIV.info
echo "" >> openfoamDNS_to_pseudoPIV.info
...
echo "DNS_crop_z0=${DNS_crop_z0}" >> openfoamDNS_to_pseudoPIV.info
...
echo "DNS_crop_Lz=${DNS_crop_Lz}" >> openfoamDNS_to_pseudoPIV.info
echo "  => DNS_crop_Rz=${DNS_crop_Rz}" >> openfoamDNS_to_pseudoPIV.info
...
set code_view_sliceZ_Ux1 = ` cat ${fic_param} | grep "code_view_sliceZ_Ux1" | awk -F'#' '{ print $1 }' `
set code_view_sliceZ_Uy1 = ` cat ${fic_param} | grep "code_view_sliceZ_Uy1" | awk -F'#' '{ print $1 }' `
...
set code_view_sliceZ_Ux2 = ` cat ${fic_param} | grep "code_view_sliceZ_Ux2" | awk -F'#' '{ print $1 }' `
set code_view_sliceZ_Uy2 = ` cat ${fic_param} | grep "code_view_sliceZ_Uy2" | awk -F'#' '{ print $1 }' `
set code_csv_sliceZ_Ux1 = ` cat ${fic_param} | grep "code_csv_sliceZ_Ux1" | awk -F'#' '{ print $1 }' `
set code_csv_sliceZ_Uy1 = ` cat ${fic_param} | grep "code_csv_sliceZ_Uy1" | awk -F'#' '{ print $1 }' `
set code_csv_sliceZ_Ux2 = ` cat ${fic_param} | grep "code_csv_sliceZ_Ux2" | awk -F'#' '{ print $1 }' `
set code_csv_sliceZ_Uy2 = ` cat ${fic_param} | grep "code_csv_sliceZ_Uy2" | awk -F'#' '{ print $1 }' `
set code_csv_sliceZ_Uz2 = ` cat ${fic_param} | grep "code_csv_sliceZ_Uz2" | awk -F'#' '{ print $1 }' `

# NOTE : code_csv_sliceY* = code_csv_sliceZ*

if ( ${code_csv_sliceZ_Uz2} == "" ) then
  set code_csv_sliceZ_Uz2 = 0
  echo "${code_csv_sliceZ_Uz2} # code_csv_sliceZ_Uz2 : =1 if CSV file showing Uz(Z=Lz/2) openfoam after smoothing" >> ${fic_param}
endif
...
cat util/calculator_PointVolumeInterpolation_model.py | \
  ...
  sed s/"RESOLUTION_Z_VALUE"/"${DNS_crop_Rz}"/g | \
  ...
  sed s/"CODE_VIEW_SLICEZ_UX1_VALUE"/"${code_view_sliceZ_Ux1}"/g | \
  sed s/"CODE_VIEW_SLICEZ_UY1_VALUE"/"${code_view_sliceZ_Uy1}"/g | \
  ...
  sed s/"CODE_VIEW_SLICEZ_UX2_VALUE"/"${code_view_sliceZ_Ux2}"/g | \
  sed s/"CODE_VIEW_SLICEZ_UY2_VALUE"/"${code_view_sliceZ_Uy2}"/g | \
  sed s/"CODE_CSV_SLICEZ_UX1_VALUE"/"${code_csv_sliceZ_Ux1}"/g | \
  sed s/"CODE_CSV_SLICEZ_UY1_VALUE"/"${code_csv_sliceZ_Uy1}"/g | \
  sed s/"CODE_CSV_SLICEZ_UX2_VALUE"/"${code_csv_sliceZ_Ux2}"/g | \
  sed s/"CODE_CSV_SLICEZ_UY2_VALUE"/"${code_csv_sliceZ_Uy2}"/g | \
  sed s/"CODE_CSV_SLICEZ_UZ2_VALUE"/"${code_csv_sliceZ_Uz2}"/g > slice.py
...
echo "YSLICE_VALUE=${Yslice}" >> slice_param.info
...
#------------------------------------------------------------------------------
echo ""
echo "First : for each openfoam file U (like 0/U), you should copy this file in 2 directories (1 and 2 for example)"
...
# CSV file with Ux,Uy,Uz in the same file : sliceZ

if ( (-e sliceZ_Ux1.csv) && (-e sliceZ_Uy1.csv) ) then
  if -e sliceZ_Uxy1.csv \rm sliceZ_Uxy1.csv
  paste sliceZ_Ux1.csv sliceZ_Uy1.csv | sed s/"\t"/","/g > sliceZ_Uxy1.csv
endif
if ( (-e sliceZ_Ux2.csv) && (-e sliceZ_Uy2.csv) ) then
  if -e sliceZ_Uxy2.csv \rm sliceZ_Uxy2.csv
  paste sliceZ_Ux2.csv sliceZ_Uy2.csv | sed s/"\t"/","/g > sliceZ_Uxy2.csv
endif
if ( (-e sliceZ_Uxy2.csv) && (-e sliceZ_Uz2.csv) ) then
  if -e sliceZ_Uxyz2.csv \rm sliceZ_Uxyz2.csv
  paste sliceZ_Uxy2.csv sliceZ_Uz2.csv | sed s/"\t"/","/g > sliceZ_Uxyz2.csv
endif
foreach xyz ( x y z )
#  if (-e sliceY_U${xyz}1.csv) \rm sliceY_U${xyz}1.csv
#  if (-e sliceY_U${xyz}2.csv) \rm sliceY_U${xyz}2.csv
end

echo ""
echo 'Cf. PNG files ("_withGrid":slice with grid, "_withoutGrid":slice without grid) :'
echo "  Ux_calculator_w*.png Ux_calculator_pointVolumeInterpolator_sliceZ_w*.png"
echo "  Uy_calculator_w*.png Uy_calculator_pointVolumeInterpolator_sliceZ_w*.png"
echo ""
echo "Cf. CSV files (1:openfoam grid, 2:gaussian smooth+crop -> uniform grid) :"
echo "  sliceZ_Uxy?.csv"
echo ""

# CSV file with Ux,Uy,Uz in the same file : sliceY

if ( (-e sliceY_Ux1.csv) && (-e sliceY_Uz1.csv) ) then
  if -e sliceY_Uxz1.csv \rm sliceY_Uxz1.csv
  paste sliceY_Ux1.csv sliceY_Uz1.csv | sed s/"\t"/","/g > sliceY_Uxz1.csv
endif
if ( (-e sliceY_Ux2.csv) && (-e sliceY_Uz2.csv) ) then
  if -e sliceY_Uxz2.csv \rm sliceY_Uxz2.csv
  paste sliceY_Ux2.csv sliceY_Uz2.csv | sed s/"\t"/","/g > sliceY_Uxz2.csv
endif
if ( (-e sliceY_Ux2.csv) && (-e sliceY_Ux2.csv) && (-e sliceY_Uy2.csv) ) then
  if -e sliceY_Uxyz2.csv \rm sliceY_Uxyz2.csv
  #paste sliceY_Ux2.csv sliceY_Uz2.csv  sliceY_Uy2.csv | sed s/"\t"/","/g  > sliceY_Uxyz2.csv
  paste sliceY_Ux2.csv sliceY_Uy2.csv  sliceY_Uz2.csv | sed s/"\t"/","/g  > sliceY_Uxyz2.csv
endif
foreach xyz ( x y z )
#  if (-e sliceY_U${xyz}1.csv) \rm sliceY_U${xyz}1.csv
#  if (-e sliceY_U${xyz}2.csv) \rm sliceY_U${xyz}2.csv
...
echo ""
echo 'Cf. PNG files ("_withGrid":slice with grid, "_withoutGrid":slice without grid) :'
echo "  Ux_calculator_w*.png Ux_calculator_pointVolumeInterpolator_sliceY_w*.png"
echo "  Uz_calculator_w*.png Uz_calculator_pointVolumeInterpolator_sliceY_w*.png"
echo ""
echo "Cf. CSV files (1:openfoam grid, 2:gaussian smooth+crop -> uniform grid) :"
echo "  sliceY_Uxz?.csv"
echo ""
...
# slice Z=cte
if (-e sliceZ_Uxy1.csv) then

  if -e sliceZ_Uxy1.txt \rm sliceZ_Uxy1.txt
  cat sliceZ_Uxy1.csv | grep -v Points | awk -F',' '{ print $1,$2 }' > sliceZ_Uxy1.txt

endif
if (-e sliceZ_Uxy2.csv) then
...
# slice Y=cte
if (-e sliceY_Uxz1.csv) then

  if -e sliceY_Uxz1.txt \rm sliceY_Uxz1.txt
  cat sliceY_Uxz1.csv | grep -v Points | awk -F',' '{ print $1,$2 }' > sliceY_Uxz1.txt

endif
if (-e sliceY_Uxz2.csv) then

  if -e XZcrop.txt \rm XZcrop.txt
  cat sliceY_Uxz2.csv | \
    grep -v Points | awk -F',' '{ print $1,$3 }' > XZcrop.txt

endif

foreach csv_file ( sliceZ_Uxy2.csv sliceY_Uxz2.csv )

  set base_file = ` echo ${csv_file} | sed s/"2\.csv"//g `
  set code_slice = ` echo ${base_file} | awk -F '_' '{ print $2 }' | sed s/U//g | tr '[A-Z]' '[a-z]' `
  set code_sliceM = ` echo ${code_slice} | tr '[a-z]' '[A-Z]' `
  set name_slice = ` echo ${base_file} | awk -F '_' '{ print $1 }' `

  if (-e ${csv_file}) then

    set Nx = `cat ${csv_file} | grep -v Points | \
      awk -F',' '{ x=$1+0. ; if ( NF!=0 ) printf ( "%.0f\n", 1000000.*x ) }' | sort -n | uniq | awk '{ x=$1+0. ; printf ( "%.6f\n",x/1000000. ) }' | wc -l `
    set Ny = `cat ${csv_file} | grep -v Points | \
      awk -F',' '{ x=$2+0. ; if ( NF!=0 ) printf ( "%.0f\n", 1000000.*x ) }' | sort -n | uniq | awk '{ x=$1+0. ; printf ( "%.6f\n",x/1000000. ) }' | wc -l `
    set Nz = `cat ${csv_file} | grep -v Points | \
      awk -F',' '{ x=$3+0. ; if ( NF!=0 ) printf ( "%.0f\n", 1000000.*x ) }' | sort -n | uniq | awk '{ x=$1+0. ; printf ( "%.6f\n",x/1000000. ) }' | wc -l `

    # gnuplot file

    set fic_gnp = slice${code_slice}.gnp
    if -e ${fic_gnp} \rm ${fic_gnp}; touch ${fic_gnp}

    # gnuplot file

    set fic_txt = ${base_file}1.txt
    ...
    if ( ${name_slice} == sliceZ ) then
      set ym = `echo ${DNS_ycyl} | awk '{ print $1-2 }' `
      set yM = `echo ${DNS_ycyl} | awk '{ print $1+2 }' `
    else
      set ym = ${DNS_zmin}
      set yM = ${DNS_zmax}
    endif
    set crop_file = ${code_sliceM}crop.txt

    echo 'set output "'${name_slice}'_grid.png"'  >> ${fic_gnp}
    if ( -e ${fic_txt} ) then
      echo 'plot [x0-2.5:x0+1.5]['${ym}':'${yM}'] "'${fic_txt}'" u 1:2 w p pt 6 lc rgb "red" title "grid1", "'${crop_file}'" u 1:2 w p pt 7 lc rgb "blue" title "grid2"' >> ${fic_gnp}
    else
      echo 'plot [x0-2.5:x0+1.5]['${ym}':'${yM}'] "'${crop_file}'" u 1:2 w p pt 7 lc rgb "blue" title "grid2"' >> ${fic_gnp}
    endif

    gnuplot ${fic_gnp}

    echo ""
    echo "New DNS grid :  ${Nx}x${Ny}x${Nz} points : Cf. ${name_slice}_grid.png"
    echo ""
...
#     endif
    ...
    set PIV_file_model_new = B0001_${code_sliceM}.dat
    ...
    if ( ${code_adim} == 1 ) then
      ...
      if ( ${name_slice} == sliceZ ) then
        ...
        cat ${csv_file} | grep -v Point | \
          awk -F',' -v d_c=${PIV_Dcyl} -v D_x0=${DNS_crop_x0} -v P_x0=${PIV_xmin} -v D_y0=${D_y0} -v P_y0=${P_y0} -v I=${IsValid} '{ \
        ...
        cat ${csv_file} | grep -v Point | \
          awk -F',' -v d_c=${PIV_Dcyl} -v D_x0=${DNS_crop_x0} -v P_x0=${PIV_xmin} -v D_y0=${D_y0} -v P_y0=${P_y0} -v I=${IsValid} '{ \
          printf("%10.6f %10.6f %s %s %d\n", ($1-D_x0+0.)+(P_x0+0.)/(d_c+0.), ($3-D_y0+0.)+(P_y0+0.)/(d_c+0.), $4, $5, I) }' > ${PIV_file_model_new}
      ...
      if ( ${name_slice} == sliceZ ) then
        ...
        cat ${csv_file} | grep -v Point | \
          awk -F',' -v d_c=${PIV_Dcyl} -v D_x0=${DNS_crop_x0} -v P_x0=${PIV_xmin} -v D_y0=${D_y0} -v P_y0=${P_y0} -v I=${IsValid} '{ \
          printf("%10.6f %10.6f %s %s %d\n", ($1-D_x0+0.)*(d_c+0.)+P_x0, ($3-D_y0+0.)*(d_c+0.)+P_y0+0., $4, $5, I) }' > ${PIV_file_model_new}
    ...
    if ( ${name_slice} == sliceZ ) then
      ...
      echo 'VARIABLES = "x [mm]", "z [mm]", "Vx [m/s]", "Vz [m/s]", "isValid"' >> ${PIV_file_model_new}
      echo 'ZONE T="Frame 0", I='${DNS_crop_Nx}', J='${DNS_crop_Nz}', F=POINT' >> ${PIV_file_model_new}
    endif
    ...
    if ((-e ${PIV_file_model}) && ( ${name_slice} == sliceZ )) then
    ...
    if ( ${name_slice} == sliceZ ) then
      ...
      echo 'set arrow from PIV_xr1,graph 0 to PIV_xr1,graph 1 lc rgb "orange" lw 4 nohead front'  >> ${fic_gnp}
      echo 'set arrow from PIV_xr2,graph 0 to PIV_xr2,graph 1 lc rgb "orange" lw 4 nohead front'  >> ${fic_gnp}
      echo 'set arrow from DNS_xr1,graph 0 to DNS_xr1,graph 1 lc rgb "green" lw 4 nohead front'  >> ${fic_gnp}
      echo 'set arrow from DNS_xr2,graph 0 to DNS_xr2,graph 1 lc rgb "green" lw 4 nohead front'  >> ${fic_gnp}
    endif

    # cylinder : black disk diameter D=1.0 or rectangle with width=1

    if ( ${name_slice} == sliceZ ) then
      echo 'set object 1 circle center 0,0 size 0.5 fc rgb "#FF00FF" fillstyle transparent pattern 2 lw 1 front'  >> ${fic_gnp}
    else
      echo 'set object 1 rectangle from -0.5,graph 0 to 0.5,graph 1 fc rgb "#FF00FF" fillstyle transparent pattern 2 lw 1 front'  >> ${fic_gnp}
    endif
    ...
    if (-e ${PIV_file_model} ) then
      ...
      echo 'set output "PIV_model'${name_slice}'_Ux.png"'  >> ${fic_gnp}
      ...
      if ( ${name_slice} == sliceZ ) then
        ...
        if ( ${code_plot} == "point" ) then
          echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
          echo '  "'${PIV_file_model}'" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):($3/'${PIV_Velocity}')  w p pt 5 ps 1 palette title "PIV model : Ux/'${PIV_Velocity}'"' >> ${fic_gnp}
        endif
        if ( ${code_plot} == "smooth" ) then
          echo 'splot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][][] \' >> ${fic_gnp}
          echo '  "tmp1.txt" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):($3/'${PIV_Velocity}')  title "PIV model : Ux/'${PIV_Velocity}'"' >> ${fic_gnp}
        endif
      ...
      echo 'set output "PIV_model'${name_slice}'_Uy.png"'  >> ${fic_gnp}
      ...
      if ( ${name_slice} == sliceZ ) then
        ...
        if ( ${code_plot} == "point" ) then
          echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
          echo '  "'${PIV_file_model}'" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):($4/'${PIV_Velocity}') w p pt 5 ps 1 palette title "PIV model : Uy/'${PIV_Velocity}'"' >> ${fic_gnp}
        endif
        if ( ${code_plot} == "smooth" ) then
          echo 'splot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][][] \' >> ${fic_gnp}
          echo '  "tmp1.txt" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):($4/'${PIV_Velocity}') title "PIV model : Uy/'${PIV_Velocity}'"' >> ${fic_gnp}
        endif
      ...
      echo 'set output "PIV_model'${name_slice}'_IsValid.png"'  >> ${fic_gnp}
      ...
      if ( ${name_slice} == sliceZ ) then
        ...
        if ( ${code_plot} == "point" ) then
          echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
          echo '  "'${PIV_file_model}'" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):5 w p pt 5 ps 1 palette title "PIV model : IsValid"' >> ${fic_gnp}
        endif
        if ( ${code_plot} == "smooth" ) then
          echo 'splot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][][] \' >> ${fic_gnp}
          echo '  "tmp1.txt" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):5 title "PIV model : IsValid"' >> ${fic_gnp}
        endif
    ...
    if ( ${code_adim} == 0 ) then
      ...
      if ( ${name_slice} == sliceZ ) then
        ...
        set Yslice = `echo ${DNS_yr1} ${DNS_yr2} | awk '{ printf("%.3f",($1+$2)/2.) }' `
        echo 'set title "PIV new (Y='${Yslice}') from gaussian smoothing of DNS ['${DNS_info}']" noenhanced' >> ${fic_gnp}
      endif
      ...
      echo 'set output "PIV_new'${name_slice}'_Ux.png"'  >> ${fic_gnp}
      if ( ${name_slice} == sliceZ ) then
        ...
        if ( ${code_plot} == "point" ) then
          echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
          echo '  "'${PIV_file_model_new}'" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):($3/'${PIV_Velocity}') w p pt 5 ps 1 palette title "PIV new : Y='${Yslice}' Ux/'${PIV_Velocity}'"' >> ${fic_gnp}
          endif
        if ( ${code_plot} == "smooth" ) then
          echo 'splot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][][] \' >> ${fic_gnp}
          echo '  "tmp2.txt" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):($3/'${PIV_Velocity}') title "PIV new : Ux/'${PIV_Velocity}'"' >> ${fic_gnp}
        endif
      ...
      if ( ${name_slice} == sliceZ ) then
       ...
       echo 'set output "PIV_new'${name_slice}'_Uy.png"'  >> ${fic_gnp}
       if ( ${code_plot} == "point" ) then
          echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][PIV_DNS_yr1-0.01:PIV_DNS_yr2+0.01] \' >> ${fic_gnp}
        ...
        echo 'set output "PIV_new'${name_slice}'_Uz.png"'  >> ${fic_gnp}
        if ( ${code_plot} == "point" ) then
          echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
          echo '  "'${PIV_file_model_new}'" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):($4/'${PIV_Velocity}') w p pt 5 ps 1 palette title "PIV new : Y='${Yslice}' Uy/'${PIV_Velocity}'"' >> ${fic_gnp}
          endif
        if ( ${code_plot} == "smooth" ) then
          echo 'splot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][][] \' >> ${fic_gnp}
          echo '  "tmp2.txt" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):($4/'${PIV_Velocity}') title "PIV new : Uy/'${PIV_Velocity}'"' >> ${fic_gnp}
        endif
      ...
      echo 'set cbrange [0:1]' >> ${fic_gnp}
      echo 'set output "PIV_new'${name_slice}'_IsValid.png"'  >> ${fic_gnp}
      ...
      if ( ${name_slice} == sliceZ ) then
        ...
        if ( ${code_plot} == "point" ) then
          echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
          echo '  "'${PIV_file_model_new}'" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):5 w p pt 5 ps 1 palette title "PIV new : IsValid"' >> ${fic_gnp}
        endif
        if ( ${code_plot} == "smooth" ) then
          echo 'splot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][][] \' >> ${fic_gnp}
          echo '  "tmp2.txt" u (($1-('${PIV_xcyl}'))/'${PIV_Dcyl}'):(($2)/'${PIV_Dcyl}'):5 title "PIV new : IsValid"' >> ${fic_gnp}
        endif
      ...
      if ( ${name_slice} == sliceZ ) then
        ...
        if ( ${code_plot} == "smooth" ) then
         ...
         if ( ${code_plot} == "point" ) then
          echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
          echo '  "'${PIV_file_model_new}'" u  ($1-('${PIV_xcyl}'/'${PIV_Dcyl}')):($2):($3) w p pt 5 ps 1 palette title "PIV new : Y='${Yslice}' Ux"' >> ${fic_gnp}
        endif
        if ( ${code_plot} == "smooth" ) then
          echo 'splot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][][] \' >> ${fic_gnp}
          echo '  "tmp2.txt" u  ($1-('${PIV_xcyl}'/'${PIV_Dcyl}')):($2):($3) title "PIV new : Ux"' >> ${fic_gnp}
        endif
      ...
      if ( ${name_slice} == sliceZ ) then
       ...
       if ( ${code_plot} == "point" ) then
        ...
        if ( ${code_plot} == "point" ) then
          echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
          echo '  "'${PIV_file_model_new}'" u ($1-('${PIV_xcyl}'/'${PIV_Dcyl}')):($2):($4) w p pt 5 ps 1 palette title "PIV new : Y='${Yslice}' Uy"' >> ${fic_gnp}
        endif
        if ( ${code_plot} == "smooth" ) then
          echo 'splot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][][] \' >> ${fic_gnp}
          echo '  "tmp2.txt" u  ($1-('${PIV_xcyl}'/'${PIV_Dcyl}')):($2):($4) title "PIV new : Uy"' >> ${fic_gnp}
        endif
      ...
      if ( ${name_slice} == sliceZ ) then
        ...
        if ( ${code_plot} == "point" ) then
          echo 'plot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][] \' >> ${fic_gnp}
          echo '  "'${PIV_file_model_new}'" u  ($1-('${PIV_xcyl}'/'${PIV_Dcyl}')):($2):5 w p pt 5 ps 1 palette title "PIV new : IsValid"' >> ${fic_gnp}
        endif
        if ( ${code_plot} == "smooth" ) then
          echo 'splot [PIV_DNS_xr1-0.01:PIV_DNS_xr2+0.01][][] \' >> ${fic_gnp}
          echo '  "tmp2.txt" u  ($1-('${PIV_xcyl}'/'${PIV_Dcyl}')):($2):5 title "PIV new : IsValid"' >> ${fic_gnp}
        endif
    ...
    if ( ${name_slice} == sliceZ ) then
      ...
      foreach U ( Ux ${Uyz} )
        ...
        convert \
          ${U}_calculator_pointVolumeInterpolator_sliceZ_withoutGrid.png \
